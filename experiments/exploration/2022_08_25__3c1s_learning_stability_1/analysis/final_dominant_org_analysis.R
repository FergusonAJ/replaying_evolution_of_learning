...
ancestor_df = read.csv('../data/ancestor_fitness.csv')
...
ggplot(df_summary, aes(x = seed_classification_factor, y = genome_length, fill = seed_classification_factor)) + 
  ...
  geom_point(mapping=aes(color=seed_classification_factor), position = position_jitter(width = .15, height = 0), size = .5, alpha = 0.8 ) + 
  geom_boxplot( width = .1, outlier.shape = NA, alpha = 0.5 ) +
  geom_hline(aes(yintercept = mean(ancestor_df$genome_length))) +
  scale_fill_manual(values = color_map) +
  scale_color_manual(values = color_map) + 
  xlab('Classification') + 
  ylab('Genome length') + 
...
ggplot(df_summary, aes(x = seed_classification_factor, y = accuracy_mean, fill = seed_classification_factor)) + 
  ...
  geom_point(mapping=aes(color=seed_classification_factor), position = position_jitter(width = .15, height = 0), size = .5, alpha = 0.8 ) + 
  geom_boxplot( width = .1, outlier.shape = NA, alpha = 0.5 ) +
  geom_hline(aes(yintercept = mean(ancestor_df$accuracy))) +
  geom_hline(aes(yintercept = max(ancestor_df$accuracy)), linetype = 'dashed') +
  geom_hline(aes(yintercept = min(ancestor_df$accuracy)), linetype = 'dashed') +
  scale_fill_manual(values = color_map) +
  scale_color_manual(values = color_map) + 
...
ggplot(df_summary, aes(x = seed_classification_factor, y = merit_mean, fill = seed_classification_factor)) + 
  ...
  geom_point(mapping=aes(color=seed_classification_factor), position = position_jitter(width = .15, height = 0), size = .5, alpha = 0.8 ) + 
  geom_boxplot( width = .1, outlier.shape = NA, alpha = 0.5 ) +
  geom_hline(aes(yintercept = mean(ancestor_df$merit))) +
  geom_hline(aes(yintercept = max(ancestor_df$merit)), linetype = 'dashed') +
  geom_hline(aes(yintercept = min(ancestor_df$merit)), linetype = 'dashed') +
  scale_fill_manual(values = color_map) +
  scale_color_manual(values = color_map) + 
  xlab('Classification') + 
  ylab('Mean merit') + 
