...
df = read.csv('../data/reps/28/dominant_lineage.csv')
remove_vec = c()
...
write.csv(seq_matrix,'../data/seq_data.csv')

library(ggplot2)
ggplot(df, aes(x = depth, y = origin_time)) + 
  geom_point()

ggplot(df, aes(x = origin_time, y = destruction_time)) + 
  geom_point() + 
