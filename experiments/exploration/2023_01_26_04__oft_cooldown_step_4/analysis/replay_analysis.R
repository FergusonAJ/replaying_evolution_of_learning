...
for(seed in c(28)){
  ...
  ggplot(df_learning_diffs, aes(x = start_depth, y = diff)) + 
    geom_line(size = 1.01) + 
    geom_point(size = 2) + 
  ...
  ggplot(df_learning_diffs, aes(xmin = start_depth, xmax = finish_depth, ymin = diff_frac-0.05, ymax = diff_frac+0.05)) + 
    geom_rect() +
