...
for(depth in c(375, 391:394, 413)){
  input_filename = paste0(input_dir, '/', depth, '/combined_mutation_classification.csv')
...
ggsave(paste0(plot_dir, '/combined_mutant_classification.pdf'), width = 9, height = 6, units = 'in')
ggsave(paste0(plot_dir, '/combined_mutant_classification.png'), width = 9, height = 6, units = 'in')
...
ggplot(df[df$depth >= 391 & df$depth <= 394,], aes(x = depth, y = frac, color = seed_classification_factor)) + 
  ...
  ylab('Fraction of mutants (up to two steps)') + 
  theme(legend.position = 'none')
ggsave(paste0(plot_dir, '/combined_mutant_classification__focal.pdf'), width = 9, height = 6, units = 'in')
ggsave(paste0(plot_dir, '/combined_mutant_classification__focal.png'), width = 9, height = 6, units = 'in')
